...
const signUp = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (isCreatedUser) return res.status(201).json(`Votre compte a bien été créé !`)
...
const updateCustomerProfile = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (req.body.email) {
      ...
      if (userExist.email === req.body.email) throw new ErrorApi(`L'email que vous avez saisi est identique à celui déjà enregistré, saisissez-en un nouveau !`, req, res, 401);

      // ? if different, I check the new email doesnt exist in database
      const isExist = await User.findUserIdentity(req.body.email)
      console.log('isExist:', isExist);
      if (isExist) throw new ErrorApi(`L'email ${isExist.email} existe déjà, saisissez-en un nouveau !`, req, res, 401);
    }
