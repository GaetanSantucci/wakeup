...
const signUp = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (isExist)
      throw new ErrorApi(
        `Le mail ${isExist.email} existe déjà !`,
        req,
        res,
        401
      )
  ...
  try {
    ...
    if (req.body.email) {
      ...
      if (userExist.email === req.body.email)
        throw new ErrorApi(
          `L'email que vous avez saisi est identique à celui déjà enregistré, saisissez-en un nouveau !`,
          req,
          res,
          401
        )
  ...
  try {
    ...
    if (req.user?.id === userId || req.user?.role === 'admin') {
      ...
      if (isUser === userId || req.user?.role === 'admin') {
        ...
        throw new ErrorApi(
          `Vous ne pouvez pas avoir accés à cette information !`,
          req,
          res,
          401
        )
...
const updateNewsletterOptin = async (req: Request, res: Response) => {
  try {
    const { email } = req.body
